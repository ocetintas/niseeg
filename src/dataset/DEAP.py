...
class DEAP(Dataset):
    ...
    def _epoch_data(self, eeg_data, labels, landmark_data):
        ...
        self.eeg = torch.tensor(eeg)
        self.landmark = torch.tensor(landmark)
        self.labels = torch.from_numpy(np.repeat(labels, self.num_segments, axis=0))  # num_segments scores per video

        assert self.eeg.shape == (eeg_data.shape[0]*self.num_segments, eeg_data.shape[1], eeg_data_per_segment), "EEG epoching went wrong!"
        assert self.landmark.shape == (landmark_data.shape[0]*self.num_segments, landmark_data.shape[1], landmark_data_per_segment), "Landmark epoching went wrong"
    ...
    def __getitem__(self, item):
        # TODO: return as  (batch, seq, feature)
        return item
